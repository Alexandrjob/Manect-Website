...
namespace Manect.Repository
{
    public class HistoryItemRepository : IHistoryItemRepository
    {
        ...
        public HistoryItemRepository(IServiceScopeFactory serviceScopeFactory,
            IStringFormatService messageFormatService)
        {
        ...
        public async Task<List<HistoryItem>> GetHistoryAsync()
        {
            ...
            //TODO: Оптимизировать.
            var executors = await dataContext.Executors.AsNoTracking()
                .Where(e => AllIds.Select(a => a.ExecutorId).Contains(e.Id)).ToListAsync();
            var projects = await dataContext.Projects.AsNoTracking()
                .Where(e => AllIds.Select(a => a.ProjectId).Contains(e.Id)).ToListAsync();
            var stages = await dataContext.Stages.AsNoTracking()
                .Where(e => AllIds.Select(a => a.StageId).Contains(e.Id)).ToListAsync();
